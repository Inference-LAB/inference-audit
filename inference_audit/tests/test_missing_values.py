...
class TestMissingValues:
    ...
    def test_min_length_zero_boundary(self):
        """min_length=0 means nothing is flagged as 'too short' -- only null/whitespace count."""
        df = pd.DataFrame({"text": ["a", "", "hello", None]})
        result = check_missing_values(df, text_col="text", min_length=0)
        assert result.details["too_short_count"] == 0
        assert result.details["null_count"] == 1
        assert result.details["whitespace_only_count"] == 1

    def test_negative_min_length_returns_none(self):
        """A negative min_length is nonsensical (all lengths are >= 0) -- must reject, not silently no-op."""
        df = pd.DataFrame({"text": ["hello world", "test"]})
        result = check_missing_values(df, text_col="text", min_length=-1)
        assert result.score is None
        assert "min_length" in result.warning

    def test_non_string_column_returns_none(self):
        """A column of numeric values must not be silently coerced via str() -- misleading results."""
        df = pd.DataFrame({"text": [123, 456, 789, 1011]})
        result = check_missing_values(df, text_col="text")
        assert result.score is None
        assert "non-string" in result.warning.lower()
        assert result.details["non_string_count"] == 4
